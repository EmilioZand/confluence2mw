...
public class confluenceConvert {
	...
	private static String getFileName(String path, String newExtension) throws IOException{
		File f = new File(path);
		String originalPath = f.getPath();
		int lastDot = originalPath.lastIndexOf(".");
		    if (lastDot != -1) {
		        return originalPath.substring(0, lastDot) + newExtension;
		    } else {
	...
	public static void main(String[] args) throws IOException {
		...
		BufferedWriter out = new BufferedWriter(new FileWriter(getFileName(args[0],".mw")));
        out.write(convert(input));
        out.close();
		}
	}
